...
export class InitialSchema1000000000000 implements MigrationInterface {
   ...
   public async up(queryRunner: QueryRunner): Promise<void> {
      ...
      await queryRunner.query(`
         CREATE TABLE usuario (
            ...
            CONSTRAINT usuario_fk_conta FOREIGN KEY (id_usuario) REFERENCES conta(id) ON DELETE CASCADE,
      ...
      await queryRunner.query(`
         CREATE TABLE alimento (
            ...
            CONSTRAINT check_grupo_alimentar CHECK (
               grupo_alimentar IN ('ONIVORO', 'VEGETARIANO', 'VEGANO')
            ),
            CONSTRAINT check_estado_alimento CHECK (
               estado_alimento IN ('CRU', 'COZIDO', 'ASSADO', 'FRITO', 'GRELHADO', 'PADRAO', 'REFOGADO')
            ),
      ...
      await queryRunner.query(`
         ...
         ON alimento (nome_alimento, estado_alimento, grupo_alimentar, marca_alimento)
      ...
      await queryRunner.query(`
         CREATE TABLE tabela_nutricional (
            ...
            CONSTRAINT tabela_nutricional_check_valores CHECK (
               ...
               qtde_saturada >= 0 AND qtde_monosaturada >= 0 AND qtde_polisaturada >= 0 AND
               qtde_trans >= 0 AND qtde_sodio >= 0 AND qtde_calcio >= 0 AND
               qtde_ferro >= 0 AND qtde_potassio >= 0 AND qtde_vitamina_a >= 0 AND
               qtde_vitamina_c >= 0 AND qtde_vitamina_d >= 0 AND qtde_vitamina_e >= 0
            ),
            CONSTRAINT tabela_nutricional_check_unidade_medida CHECK (
               unidade_medida IN (
      ...
      await queryRunner.query(`
         CREATE TABLE alimento_prato (
            ...
            kcal NUMERIC(6,1) NOT NULL,
            CONSTRAINT alimento_prato_check_valores CHECK (
               porcao_padrao > 0 AND qtde_proteina >= 0 AND
      ...
      await queryRunner.query(`
         CREATE TABLE alimento_consumido (
            ...
            CONSTRAINT alimento_consumido_check_valores CHECK (
            ...
            CONSTRAINT alimento_consumido_fk_refeicao FOREIGN KEY (id_usuario, numero_refeicao) REFERENCES refeicao(id_usuario, numero_refeicao),
      ...
      await queryRunner.query(`
         CREATE TABLE perfil (
            ...
            CONSTRAINT perfil_check_valores CHECK (
               proteina_peso >= 0 AND carboidrato_peso >= 0 AND gordura_peso >= 0 AND
               meta_proteina >= 0 AND meta_carboidrato >= 0 AND meta_gordura >= 0 AND
               tmt > 0 AND tmf > 0 AND tmb > 0
            ),
            CONSTRAINT perfil_check_nivel_atividade CHECK (
               nivel_atividade IN ('SENDENTARIO', 'LEVE', 'MODERADO', 'INTENSO', 'EXTREMO')
            ),
            CONSTRAINT perfil_check_objetivo CHECK (
               objetivo IN ('PERDA', 'MANUTENCAO', 'GANHO')
            ),
            CONSTRAINT perfil_unique_id_usuario_dt_criacao UNIQUE (id_usuario, dt_criacao_perfil),
   ...
   public async down(queryRunner: QueryRunner): Promise<void> {
      await queryRunner.query(`DROP TABLE IF EXISTS perfil CASCADE`);
      await queryRunner.query(`DROP TABLE IF EXISTS alimento_favorito CASCADE`);
      await queryRunner.query(`DROP TABLE IF EXISTS alimento_consumido CASCADE`);
      await queryRunner.query(`DROP TABLE IF EXISTS alimento_prato CASCADE`);
      await queryRunner.query(`DROP TABLE IF EXISTS prato CASCADE`);
      await queryRunner.query(`DROP TABLE IF EXISTS tabela_nutricional CASCADE`);
      await queryRunner.query(`DROP TABLE IF EXISTS codigo_de_barras CASCADE`);
      await queryRunner.query(`DROP TABLE IF EXISTS alimento CASCADE`);
      await queryRunner.query(`DROP TABLE IF EXISTS refeicao CASCADE`);
      await queryRunner.query(`DROP TABLE IF EXISTS dia CASCADE`);
      await queryRunner.query(`DROP TABLE IF EXISTS cartao CASCADE`);
      await queryRunner.query(`DROP TABLE IF EXISTS usuario CASCADE`);
      await queryRunner.query(`DROP TABLE IF EXISTS conta CASCADE`);
   }
